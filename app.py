...
def process_update(update):
    try:
        ...
        if not text:
            send_message(chat_id, "Sorry, I only handle text messages")
            return
        if text.startswith("/"):
...
def home():
    t = datetime.now() - datetime.fromisoformat(stats["started_at"])
    return jsonify({"status":"LIVE","msgs":stats["total_messages"],"users":len(stats["unique_users"]),"uptime":int(t.total_seconds())})
...
def webhook():
    try:
        process_update(request.get_json(force=True))
