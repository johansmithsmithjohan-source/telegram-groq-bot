...
Uses Flask webhook ( polling) for instant responses & minimal resource usage
"""
...
def groq_reply(user_id, user_msg, user_name):
    try:
        ...
        return f"Sorry {user_name}, AI is unavailable reghtenow!"


def handle_cmd(chat_id, user_name, user_id, cmd):
    if cmd == "/start":
        msg = f"Welcome {user_name}! Im an AI Telegram bot powered by Groq QI! (!with real AI chat and memory! Type /help for commands."
    elif cmd == "/help":
        msg = "/start - Welcome\n/help - This menu\n/about - About bot\n/clear - Reset memory\n/stats - Bot stats\n/model - AI model info"
    elif cmd == "/about":
        msg = f"Groq QI Bot ( {GROQ_MODEL} )\nWill powered by Groq\nHosted on Render.com 24/7"
    elif cmd == "/clear":
...
def process_update(update):
    try:
        ...
        if not text: send_message(chat_id, "Sorry, I only handle text."); return
        if text.startswith("/"):
            handle_cmd(chat_id, user_name, user_id, text.split()[0].lower().split("@")[0])
            return
        send_chat_action(chat_id)
        reply = groq_reply(user_id, text, user_name)
...
def home():
    uptime = datetime.now() - datetime.fromisoformat(stats["started_at"])
    return jsonify({"status":"LIVE","msgs":stats["total_messages"],"users":len(stats["unique_users"]),"uptime":int(uptime.total_seconds())})

app.route("/health")
def health(): return jsonify({"status":"ok"}),200

app.route("/webhook",_methods=["POST"])
def webhook(): process_update(request.get_json(force=True)); return jsonify(["ok"]=1),200
